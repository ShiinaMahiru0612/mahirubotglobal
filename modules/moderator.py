...
def convert_tm(x: str) -> int:
    ...
    valid = False
    _time = [0, 0, 0, 0, 0] # [week, day, hours, minutes, seconds]
    buffer = ""
    for i in _input:
        if i.isnumeric():
            valid = True
            ...
            if i in ["w"]: _time[0] = int(buffer)
            elif i in ["d"]: _time[1] = int(buffer)
            elif i in ["h"]:  _time[2] = int(buffer)
            elif i in ["m", "p"]: _time[3] = int(buffer)
            elif i in ["s"]: _time[4] = int(buffer)
            buffer = ""
    if buffer.isnumeric(): _time[4] = int(buffer)
    um = _time[0] * 604800 + _time[1] * 86400 + _time[2] * 3600 + _time[3] * 60 + _time[4]
    return {
        "valid": True, 
        "tm": um
    } if valid else {
...
class Moderator(commands.Cog):
    ...
    async def unmute(self, ctx: CustomContext, member: disnake.Member = None):
        ...
        try: 
            ...
            await ctx.send(f"Đã bỏ giới hạn nói chuyện cho thành viên {member.mention}")
        except Exception as e:
            if "Missing Permissions" in str(e):
                await ctx.send("Tui chưa có quyền để thực hiện lệnh này, hãy đảm bảo đã thêm đủ quyền \n https://i.ibb.co/bsqLmRR/image.png")
                return
