        ...
        for item in os.walk(eventdir):
            ...
            for file in files:
                ...
                try:
                    self.reload_extension(module_filename)
                    if self.pool.controller_bot == self and not self.bot_ready:
                        print(Fore.GREEN + f"{'=' * 48}\n[✅] {bot_name} - Reloaded Event {filename}.")
                    event_loadstat["reloaded"].append(f"{filename}")
                except (commands.ExtensionAlreadyLoaded, commands.ExtensionNotLoaded):
                    try:
                        self.load_extension(module_filename)
                        if self.pool.controller_bot == self and not self.bot_ready:
                            print(Fore.GREEN + f"{'=' * 48}\n[✅] {bot_name} - Loaded Event {filename}.")
                        event_loadstat["loaded"].append(f"{filename}")
                    except Exception as e:
                        if self.pool.controller_bot == self and not self.bot_ready:
                            print(Fore.RED + f"{'=' * 48}\n[❌] {bot_name} - Failed to load/reload event: {filename}")
                            raise e
